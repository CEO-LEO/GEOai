...
def _calc_swab(moisture_vv: float, bsi: float,
    ...
    #
    #    เดิม soil_air_pct = total_pore - min(soil_water_pct, total_pore) โดยที่
    ...
    #    soil_water_pct มักเกิน total_pore (28-48%) อยู่แล้ว → air ติดค่า 0% แทบ
    #    ตลอดเวลา (พบจากรายงานจริง) แถม "วัสดุดิน" ที่คำนวณจาก 100-น้ำ-อากาศ ก็ผัน
    #    ไปมาตามฝนทั้งที่เนื้อดินจริงไม่ได้เปลี่ยนตามปริมาณน้ำ
    #
    #    แก้ให้ 3 ค่านี้สอดคล้องกันจริงและรวมเป็น 100% เสมอ:
    #      วัสดุดิน  = คงที่ตามเนื้อดิน/ความอัดแน่น (BSI) ไม่ผันตามฝน
    #      น้ำ (แสดงผล) = ปริมาณน้ำที่รูพรุนเก็บได้จริง (cap ที่ total_pore)
    #      อากาศ    = พื้นที่รูพรุนที่เหลือหลังหักน้ำ — จะ >0 จริงยกเว้นตอนน้ำท่วม
    #                 ขังเต็มรูพรุนสนิท (ซึ่งตอนนั้น air=0 คือค่าที่ถูกต้องจริงๆ —
    #                 นั่นคือความหมายของ "รากฝอยขาดออกซิเจน" ที่ระบบเตือนอยู่แล้ว)
    total_pore     = max(28.0, 48.0 - max(0.0, bsi) * 22.0)
    solid_pct      = round(100.0 - total_pore, 1)
    soil_water_pct = round(min(soil_water_raw, total_pore), 1)
